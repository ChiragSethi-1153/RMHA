...
export class RabbitmqConnectionService {
  ...
  async createChannel() {
    if (!this.connection)
      throw new Error('RabbitMQ connection is not estabilished');
  ...
  async connect() {
    try {
      ...
      console.log(
        `Failed to estabilish connection to RabbitMQ: ${err.message || err}`,
      );
  ...
  incrementRedeliveryCountAndSetTTL(message: RabbitMQConsumeMessage) {
    ...
    message.properties.expiration =
      message.properties.headers['redeliveryCount'] * 2000;
